...
var ConvertBcv bot.SlashSubcommand = bot.SlashSubcommand{
	Metadata: &discordgo.ApplicationCommandOption{
		Type:        discordgo.ApplicationCommandOptionSubCommand,
		Name:        "bcv",
		Description: "Converts dollars from BCV rate to Parallel rate",
		Options: []*discordgo.ApplicationCommandOption{
			{
				Type:        discordgo.ApplicationCommandOptionNumber,
				Name:        "amount",
				Description: "Amount in USD (BCV) to convert",
				Required:    true,
			},
		},
	},
	Handler: func(s *discordgo.Session, i *discordgo.InteractionCreate, ctx *bot.BotContext) error {
		...
		embed := &discordgo.MessageEmbed{
			Title: "Conversión: BCV ➔ Paralelo",
			Color: 0x00ff00,
			Fields: []*discordgo.MessageEmbedField{
				{
					Name:   "Monto Original (BCV)",
				...
				{
					Name:   "Resultado (Paralelo)",
					Value:  fmt.Sprintf("```fix\n$ %s\n```", formatVez(resultadoParalelo)),
					Inline: true,
				},
			},
